...
def case_add(s, base_url, case_name, rich_text_content, edit_type, class_id, field_id, open_field_id, markdown_content) \
    ...
    :param rich_text_content: 富文本框编辑内容，传入rich_text_content时，edit_type需为2；参考格式：“输入内容需要用双引号括起来”，
    选择一种编辑模式，另一个编辑模式的content参数不传
    :param edit_type: 编辑模式，值包括2（富文本框编辑模式）和1（markdown编辑模式）
    :param class_id: 案例的分类id，需要双引号
    :param field_id: 案例的领域id，需要双引号
    :param open_field_id: 案例开放领域的id，需要双引号
    :param markdown_content: markdown编辑内容，传入markdown_content时，edit_type需为1； 参考格式：“需要用双引号括起来”
    :return:
    """
